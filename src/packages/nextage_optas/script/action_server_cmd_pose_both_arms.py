...
from nextage_optas.msg import CmdDualPosePickAndPlaceAction, CmdDualPosePickAndPlaceFeedback, CmdDualPosePickAndPlaceResult
...
class CmdPoseActionServer(object):
    ...
    def __init__(self, name):
        ...
        # fixed joint angle values
        self.chest_joint = rospy.get_param("~chest_joint", 0.0)
        self.head0_joint = rospy.get_param("~head0_joint", 0.0)
        self.head1_joint = rospy.get_param("~head1_joint", 0.0)
        # robot name
        # end-effector frame
        ...
        ori = builder_right_arm.add_parameter('ori', 1)
        ...
        ori_fnc = self.right_arm.get_link_quaternion_function(link=self._link_ee_right, base_link=self._link_ref)
        ori_rpy = optas.SX.zeros(3); ori_rpy[2] = ori
        ori_quat = optas.Quaternion.fromrpy(ori_rpy).getquat()
        builder_right_arm.add_equality_constraint('final_ori', ori_fnc(q_var), rhs=ori_quat)
        ...
        ori = builder_left_arm.add_parameter('ori', 1)
        # set variable boudaries
        ...
        # equality constraint on orientation
        ori_fnc = self.left_arm.get_global_link_quaternion_function(link=self._link_ee_left)
        ori_fnc = self.left_arm.get_link_quaternion_function(link=self._link_ee_left, base_link=self._link_ref)
        ori_rpy = optas.SX.zeros(3); ori_rpy[2] = ori
        ori_quat = optas.Quaternion.fromrpy(ori_rpy).getquat()
        builder_left_arm.add_equality_constraint('final_ori', ori_fnc(q_var), rhs=ori_quat)
        # optimization cost: close to nominal config
        ...
        self._feedback = CmdDualPosePickAndPlaceFeedback()
        self._result = CmdDualPosePickAndPlaceResult()
        # declare action server
        self._action_server = actionlib.SimpleActionServer(
            'cmd_pose_pick_and_place',
            CmdDualPosePickAndPlaceAction, 
            execute_cb=None,
            auto_start=False
        )
    ...
    def goal_cb(self):
        ...
        ori_R = acceped_goal.angleR
        ori_L = acceped_goal.angleL
        ...
        if (pos_L > self._pos_max).any() or (pos_L < self._pos_min).any():
            ...
            return
        # print goal request
        rospy.loginfo("%s: Request to send right arm to position (%.2f, %.2f, %.2f) with orientation (%.2f), and left arm to (%.2f, %.2f, %.2f) with orientation (%.2f) in %.1f seconds." % (
                self._name, 
        ...
        # fixing torso and head joints
        qT[0] = self.chest_joint # CHEST_JOINT0
        qT[1] = self.head0_joint # HEAD_JOINT0
        qT[2] = self.head1_joint # HEAD_JOINT1
        ### ---------------------------------------------------------
        # helper variables
...
if __name__=="__main__":
    # Initialize node
    rospy.init_node("cmd_pose_pick_and_place_server", anonymous=True)
