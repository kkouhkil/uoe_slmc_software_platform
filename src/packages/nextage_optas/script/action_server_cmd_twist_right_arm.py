...
class CmdTwistActionServer(object):
    ...
    def __init__(self, name):
        ...
        # get head limits
        q_min_head = self._K_safety_lim_q * self.head.dlim[0][0]
        q_max_head = self._K_safety_lim_q * self.head.dlim[0][1]
        dq_min_head = self.dt * self._K_safety_lim_q_dot * self.head.dlim[1][0]
        dq_max_head = self.dt * self._K_safety_lim_q_dot * self.head.dlim[1][1]
        # get head heading
        self.pos_head_fnc = self.head.get_global_link_position_function(link=self._link_head)
        J_head = self.head.get_global_link_linear_jacobian_function(link=self._link_head)
        # get head gaze position
        self.pos_gaze_fnc = self.head.get_global_link_position_function(link=self._link_gaze)
        # set up optimization builder
        builder_head = optas.OptimizationBuilder(T=1, robots=[self.head], derivs_align=True)
        # get head joint variables
        dq_var = builder_head.get_robot_states_and_parameters(self.head_name, time_deriv=1)
        dq_opt = self.head.extract_optimized_dimensions(dq_var)
        q_var = builder_head.add_parameter('q', self.ndof)
        q_opt = self.head.extract_optimized_dimensions(q_var)
        # set end-effector parameters
        pos_head = builder_head.add_parameter('pos_head', 3)
        pos_gaze = builder_head.add_parameter('pos_gaze', 3)
        # add joint position limits
        builder_head.add_bound_inequality_constraint('joint_pos_lim', q_min_head, q_opt+dq_opt, q_max_head)
        # add joint velocity limitis
        builder_head.add_bound_inequality_constraint('joint_vel_lim', dq_min_head, dq_opt, dq_max_head)
        # optimization cost: close to nominal config
        # builder
        builder_head.add_cost_term('cost_dq', optas.norm_2(pos_head-pos_gaze+J_head(q_var)@dq_var))
        # setup solver
    ...
    def timer_cb(self, event):
        ...
        if(self._correct_mux_selection):
            ...
            ### ---------------------------------------------------------
            ### head problem
            self.solver_head.reset_initial_seed({f'{self.head_name}/dq/x': self.head.extract_optimized_dimensions(dq)})
            self.solver_head.reset_parameters({
                f'{self.head_name}/dq/p': self.head.extract_parameter_dimensions(dq),
                'q': q_curr,
            ...
            if self.solver_head.did_solve():
                dq = solution[f'{self.head_name}/dq']
            else:
